"""Single-sample inference entry point for Exp-04 — SPOTER.
...
LOGGER = get_logger("exp_04_spoter.inference")
DEFAULT_CONFIG = str(Path(__file__).parent / "configs" / "default.yaml")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run SPOTER inference.")
    ...
    raise SystemExit(
        "exp_04_spoter/inference.py is a runnable skeleton; migrate the real pipeline."
    )
