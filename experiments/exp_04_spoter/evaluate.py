"""Evaluation entry point for Exp-04 — SPOTER.

Computes Top-1 / Top-5 accuracy on the EgSL-5K test split.
Reported results for this experiment: Top-1 24.55%, Top-5 44.11% (Baseline).
...
LOGGER = get_logger("exp_04_spoter.evaluate")
DEFAULT_CONFIG = str(Path(__file__).parent / "configs" / "default.yaml")


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate SPOTER.")
    ...
    _ = top_k_accuracy
    raise SystemExit(
        "exp_04_spoter/evaluate.py is a runnable skeleton; migrate the real eval loop."
    )
