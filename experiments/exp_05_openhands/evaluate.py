"""Evaluation entry point for Exp-05 — OpenHands Decoupled ST-GCN.

Computes Top-1 / Top-5 accuracy on the EgSL-5K test split.
Reported results for this experiment: Top-1 22.67%, Top-5 52.79% (Final).
...
LOGGER = get_logger("exp_05_openhands.evaluate")
DEFAULT_CONFIG = str(Path(__file__).parent / "configs" / "default.yaml")


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Decoupled ST-GCN.")
    ...
    _ = top_k_accuracy
    raise SystemExit(
        "exp_05_openhands/evaluate.py is a runnable skeleton; migrate the real eval loop."
    )
