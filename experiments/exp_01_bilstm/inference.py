"""Single-sample inference entry point for Exp-01 — BiLSTM.
...
LOGGER = get_logger("exp_01_bilstm.inference")
DEFAULT_CONFIG = str(Path(__file__).parent / "configs" / "default.yaml")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run BiLSTM inference.")
    ...
    raise SystemExit(
        "exp_01_bilstm/inference.py is a runnable skeleton; migrate the real pipeline."
    )
