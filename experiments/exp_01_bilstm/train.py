"""Training entry point for Exp-01 — BiLSTM.

Model: BiLSTM
Input: MediaPipe landmarks
Reported results: Top-1 2.19%, Top-5 7.10% (status: Rejected).
...
LOGGER = get_logger("exp_01_bilstm.train")
DEFAULT_CONFIG = str(Path(__file__).parent / "configs" / "default.yaml")


def main() -> None:
    parser = argparse.ArgumentParser(description="Train BiLSTM.")
    ...
    raise SystemExit(
        "exp_01_bilstm/train.py is a runnable skeleton; migrate the real training loop."
    )
